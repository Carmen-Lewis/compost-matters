...
exports.handler = async function() {
  ...
  try {
    ...
    const past = (pastBatch.items || []).map(simplify).sort((a, b) => new Date(a.start) - new Date(b.start)).filter((e) => e.end ? new Date(e.end) <= new Date(nowISO) : new Date(e.start) <= new Date(nowISO)).slice(-2).reverse();
    return {
      statusCode: 200,
...
//# sourceMappingURL=gcal-snapshot.js.map
